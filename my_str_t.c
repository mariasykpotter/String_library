...
static int my_str_insert_length(my_str_t* str, const char* from, size_t pos, size_t length)
{
    ...
    memmove(str->data + pos + length, str->data + pos, sizeof(char)*length);
...
int my_str_insert_c(my_str_t *str, char c, size_t pos){
    ...
    memmove(str->data + pos, str->data + pos + 1, sizeof(char)*(str->size_m - pos));
    str->data[pos] = c;
    return 0;
}
...
int my_str_substr(const my_str_t *from, my_str_t *to, size_t beg, size_t end)
{
    ...
    memcpy(to->data, from->data + beg, sizeof(char)*(end - beg));
